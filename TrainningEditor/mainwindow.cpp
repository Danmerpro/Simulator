    ...
    : QMainWindow(parent)
{
    ...
    createStatusBar();
    fileName = "";
    setWindowTitle(tr("Training Editor - Новая тренеровка"));
...
{
    fileName = QFileDialog::getSaveFileName(this,tr("Cохранить тренажную обстановку..."),".",tr("Routes Data Base files (*.xml)"));
    if(fileName.isEmpty())
        return false;
    return saveFile(fileName);
}
...
bool MainWindow::okToContinue()
{
    if(isWindowModified())
    {
        int r = QMessageBox::warning(this,tr("Поставки"),tr("База данных была изменена.\nСохранить изменения?"),QMessageBox::Yes|QMessageBox::No|QMessageBox::Cancel);
